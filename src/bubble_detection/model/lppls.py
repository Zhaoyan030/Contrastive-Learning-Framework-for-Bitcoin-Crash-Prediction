...
class LPPLS(object):
    ...
    def fit(self, max_searches=25, minimizer='Nelder-Mead', obs=None, m_range=None, w_range=None, forward=None):
        """
        Args:
        ...
        while search_count < max_searches:
            ...
            if forward is None:
                # tc_range = (max(t2 - 60, t2 - 0.5 * (t2 - t1)), min(t2 + 252, t2 + 0.5 * (t2 - t1))) # original tc range
                tc_range = (t2, t2 + (t2 - t1) / 3) # Sammy's tc range
            else:
                tc_range = (t2, t2+forward) # my tc range
            if m_range is None:
                m_range = (0.0, 1.0) # original m range
            if w_range is None:
                # w_range = (2.0, 15.0) # original w range
                w_range = (1.0, 50.0)
    ...
    def compute_indicators(self, res, filter_conditions_config=None, forward=None):
        pos_lst = []
        neg_lst = []
        pos_conf_lst = []
        neg_conf_lst = []
        ...
        if filter_conditions_config is None:
            ...
            m_min, m_max = filter_conditions_config['m_range']
            w_min, w_max = filter_conditions_config['w_range']
            O_min = filter_conditions_config['O_min']
            D_min = filter_conditions_config['D_min']
    ...
    def mp_compute_nested_fits(self, workers, window_size=80, smallest_window_size=20, outer_increment=5, inner_increment=2, max_searches=25, forward=None):
    ...
    def _func_compute_nested_fits(self, args):
        ...
        obs, window_size, n_iter, smallest_window_size, outer_increment, inner_increment, max_searches, forward = args
        ...
        for j in range(0, window_delta, inner_increment):
            ...
            if self.__class__.__name__ == 'LPPLSCMAES':
                ...
                tc, m, w, a, b, c, c1, c2, O, D = self.fit(max_searches, obs=obs_shrinking_slice, forward=forward)
            else:
                tc, m, w, a, b, c, c1, c2, O, D = self.fit(max_searches, obs=obs_shrinking_slice, forward=forward)
